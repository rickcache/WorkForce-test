...
from pages.page_approve_leave import ApproveLeave
...
def test_leave_request(driver, e_email, e_password, username, email, password, from_date, to_date, reason):
    login = EmployeeLogin(driver)
    admin_login = AdminLogin(driver)
    leave = LeaveRequest(driver)
    approve = ApproveLeave(driver)
